---
layout: default
title: MiniZinc Challenge 2024 - Docker Image Preparation
---

## Prepare a Docker image for the MiniZinc Challenge 2024
...
- Your image must use the base image of `minizinc/mznc2024:latest`
- Running `minizinc` with the default solver (i.e. without the `--solver` option) must run your
  ...
  - If you have a [solver configuration file](/doc-2.8.3/en/fzn-spec.html#solver-configuration-files)
...
### MiniZinc Challenge 2024 Docker Image

The Docker base image, which all entries must extend, was created via a [Dockerfile](dockerfiles/mznc2024/Dockerfile)
and is available at the repository `minizinc/mznc2024` with the tag `latest`.
...
```sh
docker pull minizinc/mznc2024:latest
```

The image contains a minimal installation of Ubuntu 20.04 and MiniZinc 2.8.3, which is installed in
the `/usr/local/bin` directory. It is recommended that you create a 
[Solver Configuration File](/doc-2.8.3/en/fzn-spec.html#solver-configuration-files) for your solver,
and set it as the default solver using a
[User Configuration File](/doc-2.8.3/en/command_line.html#user-configuration-files) (the details
...
```sh
docker run -it --rm minizinc/mznc2024 /bin/bash
```
...
#
# See https://www.minizinc.org/doc-2.8.3/en/command_line.html#user-configuration-files
...
They are available [here](https://github.com/MiniZinc/minizinc.github.io/tree/master/challenge2024/dockerfiles).
...
See the documentation [here](/doc-2.8.3/en/fzn-spec.html#sec-cmdline-conffiles) for more information.
...
```sh
docker run --rm minizinc/mznc2024:latest cat mzn-exec-fd > mzn-exec-fd
docker run --rm minizinc/mznc2024:latest cat mzn-exec-free > mzn-exec-free
docker run --rm minizinc/mznc2024:latest cat mzn-exec-par > mzn-exec-par
```
...
```
docker run -it --name solver minizinc/mznc2024:latest /bin/bash
```
...
Return to the [MiniZinc Challenge 2024](challenge.html) home page.
