...
These rules were last updated on 9 June {year}.
...
If an entry is a derivative work based on another solver, or includes another solver or solvers
(such as those used as part of a portfolio solver), this must be disclosed in the system description.
...
1.  Which competition CLASS(ES) the entry will compete in (see <Link href={`/challenge/${year}/rules#competition-classes`}>below</Link>
    for details).
...
entry will be run in the challenge, where it will be given a score according to the <Link href={`/challenge/${year}/rules#assessment`}>assessment procedure</Link>.
...
- FD: `minizinc -i --output-mode dzn --output-objective <model.mzn> [data.dzn]`
- FREE: `minizinc -i --output-mode dzn --output-objective -f <model.mzn> [data.dzn]`
- PAR: `minizinc -i --output-mode dzn --output-objective -p 8 <model.mzn> [data.dzn]`
...
- _timeUsed(p,s)_ - the _wall clock_ time used by the solver, or _timeLimit(p)_ if it was still
  running at the timeLimit. This time _T_ is quantized to seconds by rounding towards one:
  _max(1, floor(T / 1000))_.
...
_timeUsed(p,s') / (timeUsed(p,s') + timeUsed(p,s))_.
