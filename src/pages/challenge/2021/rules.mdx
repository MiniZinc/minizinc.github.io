...
1.  Which competition CLASS(ES) the entry will compete in (see
    [below](/challenge/2021/rules#competition-classes) for details).
...
2.  A short (1-2 pages) description of the system. This
    ...
    systems.  
    System descriptions will be posted on the [MiniZinc Challenge 2021 website](/challenge/2021).
...
[assessment procedure](/challenge/2021/rules#assessment). If the entry does not conform with the rules, or
...
<span className="text-red-600">strongly encouraged</span> to submit one or two MiniZinc
models, making use of only the global constraints included in the MiniZinc 2.5.5
library, as well as some (preferably 1) instance files for each model. The instances
should range from easy (about a minute) to hard (about 15 minutes) if possible. In
addition, the submitter should provide one "toy" instance for testing purposes. Note
that the model must conform to the problem format restrictions above.
...
Return to the [MiniZinc Challenge 2021](/challenge/2021) home page.

export default function Page({ children }) {
  return (
    <>
      <Head>
        <title>{`MiniZinc - Challenge 2021 Rules`}</title>
      </Head>
      <Header />
      <main>
        <Container>
