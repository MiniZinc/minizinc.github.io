...
1.  Which competition CLASS(ES) the entry will compete in (see
    [below](/challenge/2022/rules#competition-classes) for details).
...
2.  A short (1-2 pages) description of the system. This
    ...
    systems.  
    System descriptions will be posted on the [MiniZinc Challenge 2022 website](/challenge/2022/).
...
[assessment procedure](/challenge/2022/rules#assessment). If the entry does not conform with the rules, or
...
MiniZinc 2.6.3 library, as well as some (preferably 10 or more) instance files
for each model. The instances should range from easy (about a minute) to hard
(about 15 minutes) if possible. In addition, the submitter should provide one
"toy" instance for testing purposes.
...
Return to the [MiniZinc Challenge 2022](/challenge/2022) home page.

export default function Page({ children }) {
  return (
    <>
      <Head>
        <title>{`MiniZinc - Challenge 2022 Rules`}</title>
      </Head>
      <Header />
      <main>
        <Container>
