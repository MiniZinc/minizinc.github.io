...
These are the official rules for the MiniZinc Challenge {year}. Version 1.0.
These rules were last updated on 10 February {year}.
...
1.  Which competition CLASS(ES) the entry will compete in (see
    [below](/challenge/2024/rules#competition-classes) for details).
...
[assessment procedure](/challenge/2024/rules#assessment). If the entry does not conform with the rules, or
...
The results will be announced at <Link href={`https://cp${year}.a4cp.org`}>CP{year}</Link>. Participants are encouraged to
attend CP{year}, but are not required to in order to enter or win.
...
FlatZinc solvers must accept the following <Link href={`/doc-${challenge.minizinc}/en/fzn-spec.html#command-line-interface-and-standard-options`}>standard options</Link>, and they must be included in the supported
`stdFlags` of the <Link href={`/doc-${challenge.minizinc}/en/fzn-spec.html#sec-cmdline-conffiles`}>solver configuration</Link>:
...
MiniZinc solvers must support the following <Link href={`/doc-${challenge.minizinc}/en/command_line.html#options`}>options</Link>
...
Output from FlatZinc solver entries must conform to the <Link href={`/doc-${challenge.minizinc}/en/fzn-spec.html#output`}>FlatZinc specification for MiniZinc {challenge.minizinc}</Link>.
...
Solver entries will be run with models that are compatible with <Link href={`/doc-${challenge.minizinc}/en/`}>MiniZinc {challenge.minizinc}</Link>.
...
4.  Search annotations will be restricted to **bool_search**, **int_search**, **set_search** and
    **seq_search**.  
    ...
    label. A full specification of the available choices is given in the <Link href={`/doc-${challenge.minizinc}/en/fzn-spec.html#search-annotations`}>FlatZinc specification for MiniZinc {challenge.minizinc}</Link>.
