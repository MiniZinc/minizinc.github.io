...
## Prepare a Docker image for the MiniZinc Challenge {year}
...
- Running `minizinc` with the default solver (i.e. without the `--solver` option) must run your
  ...
  - If you have a <Link href={documentation("fzn-spec.html#solver-configuration-files", challenge.minizinc)}>solver configuration file</Link>
...
image (Ubuntu 22.04 with MiniZinc installed) which we provide. To do this, you will create a
...
### MiniZinc Challenge {year} Docker Image

The Docker base image, which all entries must extend, was created via a <Link href={`/challenge/${year}/dockerfiles/mznc${year}/Dockerfile`} raw>Dockerfile</Link>
and is available at the repository <code>minizinc/mznc{year}</code> with the tag `latest`.
...
<pre>
  <code className="language-sh">{`docker pull minizinc/mznc${year}:latest`}</code>
</pre>

The image contains a minimal installation of Ubuntu 22.04 and MiniZinc {challenge.minizinc}, which is installed in the `/usr/local/bin` directory.
It is recommended that you create a <Link href={documentation("fzn-spec.html#solver-configuration-files", challenge.minizinc)}>solver configuration file</Link> for your solver,
and set it as the default solver using a <Link href={documentation("command_line.html#user-configuration-files", challenge.minizinc)}>user configuration file</Link> (the details of how to do this are in the following section),
however the method of setting up scripts as in previous years is still available.

If you want to inspect the image via a bash shell, use the following command:

<pre>
  <code className="language-sh">{`docker run -it --rm minizinc/mznc${year} /bin/bash`}</code>
</pre>
...
<pre>
  <code className="language-docker">{`# Build Chuffed in an isolated stage called builder.
#
...
FROM minizinc/mznc${year}:latest AS builder
...
#
# See https://www.minizinc.org/doc-${challenge.minizinc}/en/command_line.html#user-configuration-files
RUN echo '\{"mzn_solver_path": ["/chuffed/share/minizinc/solvers"],' > $HOME/.minizinc/Preferences.json && \\
    echo '"tagDefaults": [["", "org.chuffed.chuffed"]]\}'           >> $HOME/.minizinc/Preferences.json`}</code>
</pre>
...
#### General Dockerfile structure

In general, a Dockerfile for the MiniZinc Challenge should have (close to) the following structure:

<pre>
  <code className="language-docker">{`FROM minizinc/mznc${year}:latest AS builder
RUN apt-get update -y && \\
...
RUN echo '{"mzn_solver_path": ["/my_solver/<msc_file_dir>"],' > $HOME/.minizinc/Preferences.json && \\
    echo '"tagDefaults": [["", "org.mysolver.mysolver"]]}'   >> $HOME/.minizinc/Preferences.json`}</code>
</pre>
...
See the documentation <Link href={documentation("fzn-spec.html#sec-cmdline-conffiles", challenge.minizinc)}>here</Link> for more information.
...
<pre>
  <code className="language-docker">{`FROM minizinc/mznc${year}
...
RUN echo '{"mzn_solver_path": ["/my-solver"],'            > $HOME/.minizinc/Preferences.json && \\
    echo '"tagDefaults": [["", "org.myorg.my_solver"]]}' >> $HOME/.minizinc/Preferences.json`}</code>
</pre>
...
<pre>
  <code className="language-sh">{`docker run --rm minizinc/mznc${year}:latest cat mzn-exec-fd > mzn-exec-fd
docker run --rm minizinc/mznc${year}:latest cat mzn-exec-free > mzn-exec-free
docker run --rm minizinc/mznc${year}:latest cat mzn-exec-par > mzn-exec-par`}</code>
</pre>

Edit them in your editor of choice, and then `COPY` them in your Dockerfile:

<pre>
  <code className="language-docker">{`FROM minizinc/mznc${year}:latest
...
COPY ./mzn-exec-par /minizinc/`}</code>
</pre>
...
<pre>
  <code className="language-sh">{`docker run -it --name solver minizinc/mznc${year}:latest /bin/bash`}</code>
</pre>
...
export default function Page({ children }) {
  return (
    <>
      <Head>
        <title>{`MiniZinc - Challenge ${year} Docker Image Preparation`}</title>
      </Head>
      <Header />
      <main>
        <Container>
