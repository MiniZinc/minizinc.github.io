...
function modifyToTal(selected_solver, selected_problem) {
  ...
  addRowText(bd, ["TOTAL", total, total_inc], 1);
  tbl.appendChild(bd);
    ...
    var color = "none";
      ...
      if (color == "none") {
        addRowText(bd, [solvers[i], 
          total_solver[i].toFixed(2), total_solver_inc[i].toFixed(2)], 1);
      } else {
        addRowTextWithColor(bd, [solvers[i], 
          total_solver[i].toFixed(2), total_solver_inc[i].toFixed(2)], 1, color, 0);
      }
    }
  }
  tbl.appendChild(bd);
...
function modifyProblem(selected_solver, selected_problem) {
  ...
  for(i = 0; i < nb_p; i++){
    ...
    row.appendChild(createTxtCol(total_problem_inc[i], "right"));
    bd.appendChild(row);
    tbl.appendChild(bd);
    bd = document.createElement('tbody');
    var j = 0;
