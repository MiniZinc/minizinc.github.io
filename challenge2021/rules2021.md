---
layout: default
title: MiniZinc Challenge 2021 - Rules
---

## MiniZinc Challenge 2021 - Rules

These are the official rules for the MiniZinc Challenge 2021. Version 1.0.  
These rules were last updated on 6 May 2021.

### Solver Submission

The MiniZinc Challenge 2021 will test solvers on problems written in MiniZinc 2.5.5.
...
Challenge 2021 image (as provided by the organisers) as its base.  
...
1.  Which competition CLASS(ES) the entry will compete in (see 
    [below](rules2021.html#competition-classes) for details).
...
2.  A short (1-2 pages) description of the system. This
    ...
    systems.  
    System descriptions will be posted on the [MiniZinc Challenge 2021 website](challenge.html).
...
[assessment procedure](rules2021.html#assessment). If the entry does not conform with the rules, or 
...
The results will be announced at [CP2021](https://cp2021.a4cp.org). Participants are encouraged to
attend CP2021, but are not required to in order to enter or win.
...
A challenge entry Docker image must use the MiniZinc Challenge 2021 image (as provided by the
...
FlatZinc solvers must accept the following [standard options](/doc-2.5.5/en/fzn-spec.html#command-line-interface-and-standard-options), and they must be included in the supported
`stdFlags` of the [solver configuration](/doc-2.5.5/en/fzn-spec.html#sec-cmdline-conffiles):
...
MiniZinc solvers must support the following [options](doc-2.5.5/en/command_line.html#options)
(by placing them in the solver configuration  `stdFlags` or in `extraFlags` for long-form options):

- `--output-mode dzn`  
  Output solutions (assignments to variables annotated with `add_to_output`) in DZN format.
...
[FlatZinc 2.5.5 specification](https://www.minizinc.org/doc-2.5.5/en/fzn-spec.html#output).
...
During the MiniZinc Challenge 2021 all instances of Docker images will run on Amazon EC2 M5 Instance
from the Amazon Web Services with the following specification:

*   Image Operating System: Ubuntu 20.04 LTS (focal)
*   Processor(s): 3.1 GHz Intel Xeon® Platinum 8175M
...
[MiniZinc 2.5.5](../doc-2.5.5/en/MiniZinc%20Handbook.pdf).  
...
2.  In order to facilitate local search entries, ideally a model should wrap symmetry breaking
    ...
    ```
    var 0..100: x :: add_to_output;
    var 0..100: y :: add_to_output;
    constraint x + y < 144;
    ...
    ```
    array[1..4] of var 0..20: start :: add_to_output;
...
3.  Each solve item must be annotated with a search strategy, such that fixing all the variables
    ...
    ```
    var 1..5: x :: add_to_output;
    var 1..5: y :: add_to_output;
    var 1..5: z :: add_to_output;
    constraint x <= y /\ y <= z;
...
4.  Search annotations will be restricted to **bool_search**, **int_search** and **seq_search**.  
    ...
    *   search style
    ...
    ```
    var 1..5: x :: add_to_output;
    var 1..10: objective :: add_to_output;
    ...
    [FlatZinc 2.5.5 specification](https://www.minizinc.org/doc-2.5.5/en/fzn-spec.html#search-annotations).

5.  All variables appearing in the search strategy (that is, variables where if all fixed, would
    allow a propagation solver to check a solution) must be annotated with `add_to_output`. 

    ```
    var 1..10: x :: add_to_output;
    var int: y = x div 2;
    var 1..3: z :: add_to_output;
    constraint y * z = 2;
    solve :: int_search([x, z], input_order, indomain_min, complete)
             satisfy;
    ```
...
The benchmarks for MiniZinc Challenge 2021 will be selected by the judges to try to examine some of 
...
of only the global constraints included in the MiniZinc 2.5.5 library, as well as some (preferably 
...
There will be an initial submission round, which will provide the organizers with an opportunity to
provide feedback on entries' compatibility with the competition hardware, compliance with these
rules, and any other matters that may arise. Submission in the initial round is not required in 
order to qualify for the final round, but it is **strongly** encouraged.
